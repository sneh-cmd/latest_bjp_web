...
const AddCoInchargeModal = ({ isOpen, onClose, boothNumber, onSave, editData = null, mode = 'create', existingMobiles = [], duplicateContextLabel = 'बुथ सह इनचार्ज', duplicateMessage = '' }) => {
  ...
  return (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-1 sm:p-4 bg-black/60 backdrop-blur-sm">
      <div className="relative w-full max-w-xs sm:max-w-md bg-white rounded-xl sm:rounded-2xl shadow-2xl overflow-hidden max-h-[95vh] overflow-y-auto">
        {/* Modal Header */}
        ...
        {/* Modal Content */}
        <form onSubmit={handleSubmit} className="p-2.5 sm:p-6 space-y-2 sm:space-y-6" style={{backgroundColor:'#f4f6ff'}} noValidate>
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-2 sm:px-4 py-1.5 sm:py-3 rounded-lg border focus:outline-none focus:bg-white transition-all text-gray-800 text-xs sm:text-base disabled:opacity-50 disabled:cursor-not-allowed"
          ...
          <div>
            ...
            <input
              ...
              value={formData.name}
              onChange={handleInputChange}
              className="w-full px-2 sm:px-4 py-1.5 sm:py-3 rounded-lg border focus:outline-none focus:bg-white transition-all text-gray-800 text-xs sm:text-base"
              style={{backgroundColor: '#f0f4ff', borderColor: '#103a94'}}
              onFocus={(e) => e.target.style.borderColor = '#103a94'}
              onBlur={(e) => e.target.style.borderColor = '#103a94'}
              ...
              {photoPreview ? (
                <div className="relative">
                  <label
                    ...
                    className="block w-full h-20 sm:h-40 rounded-lg border overflow-hidden flex items-center justify-center bg-gray-50 cursor-pointer transition-all hover:bg-gray-100"
                ...
                <label
                  htmlFor="co-incharge-photo-input"
                  className="w-full h-16 sm:h-32 rounded-lg border flex flex-col items-center justify-center cursor-pointer transition-all"
                  style={{backgroundColor: '#f0f4ff', borderColor: '#103a94'}}
                  onMouseEnter={(e) => e.target.style.backgroundColor = '#e6f0ff'}
                  onMouseLeave={(e) => e.target.style.backgroundColor = '#f0f4ff'}
                >
                  <div className="text-center">
                    <svg className="w-4 h-4 sm:w-8 sm:h-8 mx-auto mb-0.5 sm:mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" style={{color: '#103a94'}}>
          ...
          {/* Action Buttons */}
          <div className="flex flex-col sm:flex-row space-y-1.5 sm:space-y-0 sm:space-x-3 pt-1.5 sm:pt-4">
            <button
              type="button"
              onClick={onClose}
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex-1 text-white font-semibold py-1.5 sm:py-3 px-2.5 sm:px-4 rounded-lg sm:rounded-xl transition-all flex items-center justify-center shadow-sm hover:shadow-md text-xs sm:text-base disabled:opacity-50 disabled:cursor-not-allowed"
