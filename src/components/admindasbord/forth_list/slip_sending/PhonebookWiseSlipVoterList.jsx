...
const PhonebookWiseSlipVoterList = ({ navigation }) => {
  ...
  const selectedUserName = state?.name || 'फोनबुक पर्ची रिपोर्ट'
  ...
  const fetchVoters = useCallback(async () => {
    try {
      ...
      if (!selectedUserId) {
        throw new Error('यूजर आईडी नहीं मिला')
      }

      const panelApiUrl = localStorageManager.getApiUrl()
      const response = await phonebook_wise_slip_sending(selectedUserId, panelApiUrl)
      ...
      console.error('Error fetching phonebook wise slip voters:', err)
...
export default PhonebookWiseSlipVoterList
