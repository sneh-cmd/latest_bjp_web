...
import { displayDateWiseCallCenterSurveyVoter } from '../../../../apidata.jsx'
...
const getSurveyStatusCode = (item) => {
  // Accept multiple possible fields that APIs might return
  return (
    item.voter_status ||
    item.status ||
    item.survey_status ||
...
const statusMatchesFilter = (item, filterKey) => {
  ...
  const original = String(getSurveyStatusCode(item) || '').trim()
  const raw = original.toUpperCase()
  if (!original) return false

  // Also accept common Hindi labels and minor spelling variants
  switch (filterKey) {
    case 'positive':
      return (
        raw === 'P' ||
        raw === 'POSITIVE' ||
        original.includes('पॉजिटिव')
      )
    case 'negative':
      return (
        raw === 'N' ||
        raw === 'NEGATIVE' ||
        original.includes('नेगेटिव')
      )
    case 'doubtful':
      return (
        raw === 'D' ||
        raw === 'DOUBTFUL' ||
        original.includes('डाउटफुल')
      )
    case 'none':
      return (
        raw === 'C' ||
        raw === 'CANT_SAY' ||
        raw === 'NONE' ||
        original.includes('कुछ नहीं')
      )
    case 'received_not':
      return (
        raw === 'NR' ||
        raw === 'NOT_RECEIVED' ||
        original.includes('रिसीव नहीं हुई')
      )
    case 'wrong_mobile':
      return (
        raw === 'WM' ||
        raw === 'WRONG_MOBILE' ||
        original.includes('ग़लत मोबाइल')
      )
    default:
      return true
  }
...
// Order taken from mobile screenshot: पॉजिटिव, नेगेटिव, डाउटफुल, कुछ नहीं, रिसीव नहीं हुई, ग़लत मोबाइल, टोटल सर्वे
const filterOptions = [
...
const DateWiseCallVoterList = ({ navigation }) => {
  const { navigate, state } = navigation
  const selectedDate = state?.selectedDate || state?.date || ''
  ...
  const fetchVoters = useCallback(async () => {
    ...
    try {
      ...
      const response = await displayDateWiseCallCenterSurveyVoter(selectedDate, panelApiUrl)
      const records = normalizeResponse(response)

      setVoters(records)
    } catch (err) {
      console.error('Error fetching call center voters for date:', selectedDate, err)
...
export default DateWiseCallVoterList
