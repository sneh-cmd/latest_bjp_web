...
import { displayBoothWiseCallCenterSurveyVoter } from '../../../../apidata.jsx'
...
// Map status filter key to one or more underlying codes
...
const CallCenterBoothVoterList = ({ navigation }) => {
  const { navigate, state } = navigation
  const boothNo = state?.boothNo || state?.booth_no || state?.id || ''
  ...
  const fetchVoters = useCallback(async () => {
    if (!boothNo) {
      setError('बूथ नं. उपलब्ध नहीं है')
      setLoading(false)
      return
    }
    ...
    try {
      ...
      const response = await displayBoothWiseCallCenterSurveyVoter(boothNo, panelApiUrl)
      const records = normalizeResponse(response)

      setVoters(records)
    } catch (err) {
      console.error('Error fetching call center voters for booth:', boothNo, err)
  ...
  const filteredVoters = useMemo(() => {
    ...
    // Text search (name, mobile, idcard, serial)
    if (searchQuery.trim()) {
  ...
  const handleCallAction = (contact) => {
    if (!contact || contact === '-' || String(contact).trim() === '') {
  ...
  const handleFamilyAction = (voter) => {
    // Navigate to family screen with voter info (id and name)
    if (!voter) {
  ...
  const handleCheckModal = (/* optional voter param */) => {
    setValidationMessage('मोबाइल नंबर उपलब्ध नहीं है।')
    setValidationOpen(true)
  }
  ...
  return (
    <div className="flex flex-col w-full h-screen overflow-hidden" style={{ backgroundColor: '#e5e8ff' }}>
      <PageHeader
        title={boothNo ? `बूथ नं : ${boothNo}` : 'बूथ नं'}
        onBack={handleBack}
...
export default CallCenterBoothVoterList
