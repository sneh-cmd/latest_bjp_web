...
import { displayRationCardWiseSurveyDash } from '../../../../apidata'

const RationCardWiseSurvey = ({ navigation }) => {
  ...
  // Fetch data from API
  const fetchRationCardWiseData = useCallback(async () => {
    try {
      setLoading(true)
      setError(null)
      
      const panelApiUrl = localStorage.getItem('panelApiUrl') || 'http://ntmc2.mhbjplok.com/webservice.asmx'
      const response = await displayRationCardWiseSurveyDash(panelApiUrl)
      
      if (response && Array.isArray(response)) {
        setRationCardData(response)
      } else {
        setRationCardData([])
      }
    } catch (err) {
      console.error('Error fetching ration card wise survey data:', err)
  ...
  const filteredData = rationCardData.filter(item => {
    if (!searchQuery.trim()) return true
    const query = searchQuery.toLowerCase()
    return item.cardType.toLowerCase().includes(query)
  })

  const totalCount = filteredData.length

  const handleCardClick = (item, index) => {
    navigate('/ration-card-detail', {
      categoryData: {
        ...item,
        index
      }
  ...
  return (
    ...
    >
      ...
      <div className="sticky top-0 z-20 w-full px-3 sm:px-4 md:px-6 py-2.5 sm:py-3 flex items-center justify-between shadow-md" style={{ backgroundColor: '#102463' }}>
        ...
        <h1 className="text-white text-base sm:text-lg md:text-xl font-bold text-center flex-1 px-2">राशन कार्ड अनुसार सर्वे</h1>
        ...
        <div className="sticky top-[60px] sm:top-[64px] z-10 w-full px-3 sm:px-4 md:px-6 py-2 bg-white border-b shadow-md">
          <div className="flex items-center space-x-2">
            <input
              ...
              placeholder="कार्ड प्रकार से खोजें..."
      ...
      <div className="w-full px-3 sm:px-4 md:px-6 py-3 sm:py-4 pb-20 space-y-3 sm:space-y-4 mb-7">
        {loading ? (
          ...
          <div className="text-center py-8 text-red-500">
            ...
            <button
              onClick={fetchRationCardWiseData}
          ...
          >
            <div className="p-3 sm:p-4">
              ...
              <div className="flex items-center justify-between">
                {/* Left Side: Avatar and Card Type Info */}
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <button 
                    ...
                    onClick={(e) => {
                      ...
                      console.log('Call action for:', item.cardType)
                    }}
                    title="Call"
                  >
...
export default RationCardWiseSurvey
