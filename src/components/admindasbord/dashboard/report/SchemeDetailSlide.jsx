...
const SchemeDetailSlide = ({ navigation }) => {
  ...
  const fetchVoters = useCallback(async () => {
    try {
      ...
      let categoryId = categoryData.id || categoryData.categoryId || categoryData.category_id || categoryData.scheme_id
      ...
      if (!categoryId) {
        ...
        } else if (categoryData.schemeName) {
          // As last resort, try using scheme name (API might accept it)
          categoryId = categoryData.schemeName
        }
      }
      
      console.log('Scheme Data:', categoryData)
      console.log('Scheme ID (using):', categoryId)
      
      if (!categoryId) {
        console.error('Scheme ID not found in:', categoryData)
        setError('Scheme ID not found. Please check console for details.')
        setLoading(false)
        return
      }

      const panelApiUrl = localStorage.getItem('panelApiUrl') || 'http://ntmc2.mhbjplok.com/webservice.asmx'
      console.log('Fetching voters for scheme ID:', categoryId, 'from:', panelApiUrl)
      
      // Using scheme wise survey voter API
      const response = await displaySchemeWiseSurveyVoter(categoryId, panelApiUrl)
      ...
      console.error('Error fetching scheme wise survey voters:', err)
  ...
  useEffect(() => {
    const storedActiveTab = sessionStorage.getItem('schemeDetailActiveTab')
    if (storedActiveTab) {
      setActiveTab(storedActiveTab)
      sessionStorage.removeItem('schemeDetailActiveTab')
    }
  }, [])
  ...
  const handleFamilyNavigation = (voter) => {
    ...
    sessionStorage.setItem('schemeDetailActiveTab', activeTab)
  ...
  return (
    ...
    >
      <div className="sticky top-0 z-20">
        <PageHeader
          title={categoryData.schemeName || 'विवरण'}
          onBack={handleBack}
        ...
        {loading ? (
          ...
          <div className="text-center py-8 text-red-500">
            ...
            <div className="mt-2 text-xs text-gray-600">
              <p>Scheme Data: {JSON.stringify(categoryData, null, 2)}</p>
            </div>
            <button
              onClick={fetchVoters}
...
export default SchemeDetailSlide
