...
import { displayEducationWiseSurveyDash } from '../../../../apidata'

const EducationProfessionWiseSurvey = ({ navigation }) => {
  const { navigate } = navigation
  const [showSearch, setShowSearch] = useState(false)
  const [searchQuery, setSearchQuery] = useState('')
  const [educationProfessionData, setEducationProfessionData] = useState([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState(null)
  
  // Fetch data from API
  const fetchEducationWiseData = useCallback(async () => {
    try {
      setLoading(true)
      setError(null)
      
      const panelApiUrl = localStorage.getItem('panelApiUrl') || 'http://ntmc2.mhbjplok.com/webservice.asmx'
      const response = await displayEducationWiseSurveyDash(panelApiUrl)
      
      if (response && Array.isArray(response)) {
        setEducationProfessionData(response)
      } else {
        setEducationProfessionData([])
      }
    } catch (err) {
      console.error('Error fetching education wise survey data:', err)
  ...
  const filteredData = educationProfessionData.filter(item => {
    if (!searchQuery.trim()) return true
    const query = searchQuery.toLowerCase()
    return (item.category || '').toLowerCase().includes(query)
  })

  // Calculate total count of filtered categories
  const totalCount = filteredData.length || 0

  const handleCardClick = (item, index) => {
    navigate('/education-profession-detail', {
      categoryData: {
        ...item,
        index
      }
  ...
  return (
    ...
    >
      ...
      <div className="sticky top-0 z-20 w-full px-3 sm:px-4 md:px-6 py-2.5 sm:py-3 flex items-center justify-between shadow-md" style={{ backgroundColor: '#102463' }}>
        ...
        <h1 className="text-white text-base sm:text-lg md:text-xl font-bold text-center flex-1 px-2">शिक्षा/व्यवसाय अनुसार सर्वे</h1>
...
w-full px-3 sm:px-4 md:px-6 py-3 sm:py-4 pb-20 space-y-3 sm:space-y-4 mb-7">
        {loading ? (
          ...
          <div className="text-center py-8 text-red-500">
            ...
            <button
              onClick={fetchEducationWiseData}
          ...
          >
            <div className="p-3 sm:p-4">
              {/* Mobile Card Layout - Similar to Image */}
              <div className="flex items-center justify-between">
                {/* Left Side: Avatar and Category Info */}
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <button 
                    ...
                    onClick={(e) => {
                      ...
                      console.log('Call action for:', item.category)
                    }}
                  >
...
export default EducationProfessionWiseSurvey
