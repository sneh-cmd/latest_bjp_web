...
const BoothPramukhSurvey = ({ navigation }) => {
  ...
  const fetchSurveyData = useCallback(async () => {
    try {
      ...
      // survey_by_type: "bp" for Booth Pramukh
      const data = await displayTypeWiseUserListFromSurvey('bp', '')
      
      console.log('Booth Pramukh Survey Data:', data)
      setSurveyData(Array.isArray(data) ? data : [])
    } catch (err) {
      console.error('Error fetching Booth Pramukh survey data:', err)
  ...
  const handleCardClick = (item) => {
    ...
    console.log('Navigating to BoothPramukhDetailSlide with item:', item)
  ...
  const fetchUnsurveyedData = useCallback(async () => {
    try {
      ...
      // Fetch data from no_survey_user_by_type API with type="BP"
      const data = await noSurveyUserByType('BP')
      
      console.log('Unsurveyed Booth Pramukh Data:', data)
  ...
  const createExcelFile = () => {
    try {
      ...
      const title = 'सर्वे नहीं किए हुए बूथ प्रमुख'
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Unsurveyed Booth Pramukh')
            ...
            <div className="pl-2 sm:pl-5 pr-2 sm:pr-4 py-2 sm:py-4">
              ...
              <div className="grid grid-cols-2 gap-1.5 sm:gap-3 mb-2 sm:mb-3">
                ...
                <div className="rounded-md sm:rounded-lg p-1 text-center border">
                  ...
                  <div className="text-sm sm:text-lg md:text-xl font-bold">1</div>
                </div>
...
export default BoothPramukhSurvey
