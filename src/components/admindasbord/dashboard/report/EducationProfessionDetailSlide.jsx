...
import { displayEducationWiseSurveyVoter } from '../../../../apidata'
import VoterCard from '../../common/VoterCard.jsx'
import ValidationModal from '../../modals/ValidationModal.jsx'

const EducationProfessionDetailSlide = ({ navigation }) => {
  ...
  const fetchVoters = useCallback(async () => {
    try {
      ...
      let categoryId = categoryData.id || categoryData.categoryId || categoryData.category_id || categoryData.education_id || categoryData.profession_id
      ...
      if (!categoryId) {
        ...
        if (categoryData.index !== undefined) {
          ...
          // As last resort, try using category name (API might accept it)
          categoryId = categoryData.category
        }
      }
      
      console.log('Category Data:', categoryData)
      console.log('Category ID (using):', categoryId)
      
      if (!categoryId) {
        console.error('Category ID not found in:', categoryData)
        setError('Category ID not found. Please check console for details.')
        setLoading(false)
        return
      }

      const panelApiUrl = localStorage.getItem('panelApiUrl') || 'http://ntmc2.mhbjplok.com/webservice.asmx'
      console.log('Fetching voters for category ID:', categoryId, 'from:', panelApiUrl)
      
      const response = await displayEducationWiseSurveyVoter(categoryId, panelApiUrl)
      ...
      console.error('Error fetching education wise survey voters:', err)
  ...
  useEffect(() => {
    const storedActiveTab = sessionStorage.getItem('educationProfessionDetailActiveTab')
    if (storedActiveTab) {
      setActiveTab(storedActiveTab)
      sessionStorage.removeItem('educationProfessionDetailActiveTab')
    }
  }, [])
  ...
  const filteredVoters = useMemo(() => {
    ...
    const filtered = voters.filter(voter => {
      if (!voter.voterStatus || voter.voterStatus.toString().trim() === '') {
        // Skip voters that don't have a valid status
        return false
      }
  ...
  const handleBack = () => {
    navigate('/education-profession-wise-survey')
  }
  ...
  const handleFamilyNavigation = (voter) => {
    ...
    sessionStorage.setItem('educationProfessionDetailActiveTab', activeTab)
        ...
        <div className="px-2 sm:px-4 py-2 sm:py-3 shadow-md" style={{ backgroundColor: '#102463' }}>
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2 sm:space-x-3">
              ...
              <h1 className="text-white text-base sm:text-lg font-semibold">{categoryData.category || 'विवरण'}</h1>
            </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-8 text-red-500">
            ...
            <div className="mt-2 text-xs text-gray-600">
              <p>Category Data: {JSON.stringify(categoryData, null, 2)}</p>
            </div>
            <button
              onClick={fetchVoters}
...
export default EducationProfessionDetailSlide
