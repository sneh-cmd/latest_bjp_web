...
const PhonebookSurvey = ({ navigation }) => {
  ...
  const fetchSurveyData = useCallback(async () => {
    try {
      ...
      // survey_by_type: "" (empty) and survey_from: "PH" for Phonebook
      const data = await displayTypeWiseUserListFromSurvey('', 'PH')
      
      console.log('Phonebook Survey Data:', data)
      setSurveyData(Array.isArray(data) ? data : [])
    } catch (err) {
      console.error('Error fetching Phonebook survey data:', err)
  ...
  const handleCardClick = (item) => {
    ...
    console.log('Navigating to PhonebookDetailSlide with item:', item)
    navigate('/phonebook-detail-slide', {
      categoryData: item
    })
  }
...
export default PhonebookSurvey
