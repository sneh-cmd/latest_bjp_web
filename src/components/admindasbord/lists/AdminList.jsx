  ...
  const handleUpdateAdmin = async (data) => {
    try {
      ...
      const payload = {
        ...
        name: data.name || '',
        mobile_no: data.mobile || '',
        photo: data.photo || adminToUpdate?.photo || '',
    ...
    <div className={`relative w-screen h-screen overflow-hidden transition-all duration-700 ${
      ...
      <div className="relative z-10 h-full flex flex-col">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-2 sm:gap-3 w-full sm:w-auto">
          ...
          >
            <svg className="w-4 h-4 sm:w-5 sm:h-5" style={{color: '#0d2f7a'}} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
            </svg>
            <span className="text-xs sm:text-sm font-medium" style={{color: '#0d2f7a'}}>एडमिन बनाएं</span>
          </button>
        </div>
        </div>
