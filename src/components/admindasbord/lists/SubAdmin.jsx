...
const SubAdmin = ({ navigation }) => {
  ...
  // Helper function to check if subAdmin is active based on lastLogin
  const isActive = (subAdmin) => {
    return subAdmin.lastLogin && subAdmin.lastLogin.toString().trim() !== ''
  }
  ...
  const toggleStatus = (subAdminId) => {
    ...
    console.log('Status toggle requested for subAdmin:', subAdminId, 'but status is now based on lastLogin from API')
  }
        ...
        >
          <div className="flex items-center space-x-3 sm:space-x-4">
            ...
            <div className="flex flex-col items-center space-y-1 sm:space-y-2">
              ...
              >
                {isActive(subAdmin) ? 'Active' : 'Inactive'}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="px-2 sm:px-4 py-3 sm:py-4 flex flex-col sm:flex-row items-center justify-end flex-shrink-0 shadow-lg space-y-2 sm:space-y-0" style={{backgroundColor: '#102463'}}>
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            >
              ...
              <span className="text-xs sm:text-sm font-medium" style={{color: '#102463'}}>सब ऐडमिन बनाएं</span>
            </button>
          </div>
        </div>
